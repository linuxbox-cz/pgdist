...
log "mkdir -p /usr/share/pgdist"
mkdir -p /usr/share/pgdist

log "mkdir -p /usr/share/pgdist/install"
mkdir -p /usr/share/pgdist/install
...
python "${PATH_PGDIST_SRC}/pgdist.py" add "${PATH_TABLES}/d1_table_1.sql" "${PATH_TABLES}/d1_table_2.sql"

log_pgdist "status"
python "${PATH_PGDIST_SRC}/pgdist.py" status

log_pgdist "rm ${PATH_TABLES}/d1_table_1.sql ${PATH_TABLES}/d1_table_2.sql"
python "${PATH_PGDIST_SRC}/pgdist.py" rm "${PATH_TABLES}/d1_table_1.sql" "${PATH_TABLES}/d1_table_2.sql"
...
python "${PATH_PGDIST_SRC}/pgdist.py" add "${PATH_SCHEMAS}/d1_schema_1.sql" "${PATH_TABLES}/d1_table_1.sql" "${PATH_TABLES}/d1_table_2.sql" "${PATH_FUNCTIONS}/d1_function_1.sql"
...
python "${PATH_PGDIST_SRC}/pgdist.py" add "${PATH_INDEXES}/d1_index_1.sql" "${PATH_CONSTRAINTS}/d1_constraint_1.sql" "${PATH_TRIGGERS}/d1_trigger_1.sql" "${PATH_VIEWS}/d1_view_1.sql" "${PATH_DATA}/d1_data_1.sql"
...
python "${PATH_PGDIST_SRC}/pgdist.py" role-add pgdist_test_role_1 login password
...
python "${PATH_PGDIST_SRC}/pgdist.py" role-add pgdist_test_role_2 login password
...
python "${PATH_PGDIST_SRC}/pgdist.py" require-add pgdist_test_project_2 https://github.com/TadeasPopov/pgdist_test_project.git master

log_pgdist "require-rm pgdist_test_project_2"
python "${PATH_PGDIST_SRC}/pgdist.py" require-rm pgdist_test_project_2
...
python "${PATH_PGDIST_SRC}/pgdist.py" data-add pgdist_test_schema.test_table_1 id
