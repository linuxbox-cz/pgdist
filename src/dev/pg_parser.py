...
def parse(dump_stream):
	...
	for comment, command in tokens:
		...
		x = re.match(r"REVOKE .* ON FUNCTION (?P<name>\S+)\(.*\) FROM (?P<role>\S+);$", command)
		if x:
			args = remove_default(x.group('args'))
			project.functions[schema(set_schema, x.group('name')) + str(args)].revoke.append(command)
			continue

		x = re.match(r"GRANT .* ON FUNCTION (?P<name>\S+)\(.*\) TO (?P<role>\S+);$", command)
		if x:
			args = remove_default(x.group('args'))
	...
	# ######################################
		...
		logging.warning("Parser warning, unknown command: %" % (command,))
